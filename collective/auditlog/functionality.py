...
class AuditLogChecker():
    ...
    def checkAction(self, action, context, request):
        ...
        if action == "added":
            type = context.Type()
            if "Web Form" in type and \
                    context.portal_type.lower() in context.absolute_url():
                return False
        if action == "rename" and "Web Form" in context.Type():
            ...
            if (comments == 'Initial revision' or 'Create' in trans) and \
                    len(indexes) == 0 and len(history) <= 2:
                action = "added"
        return action
