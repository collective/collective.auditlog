## Script (Python) "sync_save_action"
...
#TODO - find a way to grab all types that can be added to a Web Form without hard coding them. 
...
#If URL is given, remove any parameters or views applied, then get obj from portal_catalog by shortname. 
if url:
  if "?" in url:
      path = url.split('?')
      url = path[0]
...
#Final action of saving entry into DB. Also removes copy_of_ and sets Copy field to True.
#This is so reporting can show checked out versions with the original if sorting/filter by shortname,
#but still provides indicator that is a copy. 
def takeAction(info):
 if not info['cancel']:
...
#Get info of object and store in info dict
def getObjInfo(obj):
  try:
...
#If action is on Form Field, cycles up to find the parent Form, and uses its information as the object modified.
def checkIsForm(obj):
  n = len(context.getPhysicalPath())
  info['field'] = obj.getId()
  if '.%s.'% DateTime().strftime('%Y-%m-%d') in info['field']:
    info['field'] = 'field_uid: %s' % obj.UID()
  while n>0:
   try:
