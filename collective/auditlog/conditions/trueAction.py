...
from plone.app.iterate.interfaces import IWorkingCopy, IBaseline, ICheckinCheckoutTool
...
class ITrueActionCondition(Interface):
	...
	action = schema.TextLine(title=_(u"True Action"),
				description=_(u"""The action to be tested as true. Type one of the keywords into the text box to test that action.
							The available actions are ${delete}, ${move}, and ${rename}."""),
				required=True)

class TrueActionCondition(SimpleItem):
	"""The actual persistent implementation of the true action condition element"""
	...
	def __call__(self):
	  ...
	  if "move" in action:
		try:
			...
			if oldParent and oldParent != None and newParent and newParent != None and oldParent != newParent and oldName == newName:
				trueAction = True
		except:
			trueAction = False
...
class TrueActionAddForm(AddForm):
	...
	description = ("""A True Action condition makes the rule apply only to content when the specified action truly happens (ie ${delete} is only True after
			the content has been Confirmed for Link Integrity.)""")
